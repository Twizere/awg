...
// WireGuard includes
require_once('awireguard/includes/wg.inc');
require_once('awireguard/includes/wg_guiconfig.inc');
		...
		switch ($_POST['act']) {
			case 'save':
				...
				if (empty($input_errors)) {
					...
					// Save was successful
					header('Location: /awg/vpn_wg_tunnels.php');
				}
...
$pglinks = array("", "/awg/vpn_wg_tunnels.php", "/awg/vpn_wg_tunnels.php", "@self");

$tab_array = array();
$tab_array[] = array(gettext("Tunnels"), true, "/awg/vpn_wg_tunnels.php");
$tab_array[] = array(gettext("Peers"), false, "/awg/vpn_wg_peers.php");
$tab_array[] = array(gettext("Settings"), false, "/awg/vpn_wg_settings.php");
$tab_array[] = array(gettext("Status"), false, "/awg/status_wireguard.php");
...
events.push(function() {
	...
	$('#genkeys').click(function(event) {
		if ($('#privatekey').val().length == 0 || confirm(<?=json_encode($genKeyWarning)?>)) {
			ajaxRequest = $.ajax({
				url: '/awg/vpn_wg_tunnels_edit.php',
				type: 'post',
				data: {act: 'genkeys'},
...
<?php
include('awireguard/includes/wg_foot.inc');
include('foot.inc');
?>
